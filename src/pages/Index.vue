<template>
    <Layout>
        <!-- <g-image class="greet-image" src="../../uploads/covers/computer-desktop.jpg" fit="cover" /> -->
        <div class="welcome-text">
            <h1>
                Welcome!
                <br>Come Break Some Stuff!!!
            </h1>
            <p>(Insights on modern web and application development)</p>
        </div>
        <hr>
        <h2>Recent Articles</h2>
        <b-card-group deck class="recent-articles mb-4">
            <b-card v-for="item in $page.allBlogPost.edges" :key="item.node.id" img-top>
                <g-link :to="item.node.path" class="read-more">
                    <g-image
                        class="card-img-top"
                        :src="item.node.image.src"
                        :alt="item.node.imageAlt"
                        fit="cover"
                    ></g-image>
                    <h4 class="card-title">{{item.node.title}}</h4>
                </g-link>
                <b-card-text>{{item.node.excerpt}}</b-card-text>
                <g-link :to="item.node.path" class="read-more">Read Article</g-link>
            </b-card>
        </b-card-group>
        <g-link to="/blog">
            View All Articles
